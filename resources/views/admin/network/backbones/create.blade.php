...
@section('title', 'Tambah Backbone Baru')
    ...
    <div class="lg:pl-64">
        ...
        <div class="p-6">
            <div class="mb-6 flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Tambah Kabel Backbone</h1>
                    <p class="text-gray-600 dark:text-gray-400 mt-1">Daftarkan segmen atau jalur utama kabel FO baru</p>
                </div>
            ...
            <div class="bg-white dark:bg-slate-700 rounded-xl shadow-md p-6 max-w-2xl">
                <form action="{{ route('admin.network.backbones.store') }}" method="POST">
                    @csrf

                    <div class="space-y-5">
                        <div>
                            <label for="kode_backbone" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Kode Backbone</label>
                            <input type="text" name="kode_backbone" id="kode_backbone" value="{{ old('kode_backbone') }}" placeholder="Contoh: BB-KDR-NODEA-01"
                        ...
                        <div>
                            ...
                            <input type="text" name="nama_jalur" id="nama_jalur" value="{{ old('nama_jalur') }}" placeholder="Contoh: STO Kediri ke Node Gurah"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="kapasitas_core" id="kapasitas_core" value="{{ old('kapasitas_core') }}" placeholder="Contoh: 24"
                            ...
                            <div>
                                ...
                                <input type="number" name="core_terpakai" id="core_terpakai" value="{{ old('core_terpakai', 0) }}" placeholder="Contoh: 12"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" step="0.01" name="panjang_kabel" id="panjang_kabel" value="{{ old('panjang_kabel') }}" placeholder="Contoh: 1500"
                            ...
                            <div>
                                ...
                                <select name="tipe_kabel" id="tipe_kabel" 
                                    ...
                                    <option value="aerial" {{ old('tipe_kabel') == 'aerial' ? 'selected' : '' }}>Aerial (Udara/Tiang)</option>
                                    <option value="underground" {{ old('tipe_kabel') == 'underground' ? 'selected' : '' }}>Underground (Tanam/Duct)</option>
                                    <option value="submarine" {{ old('tipe_kabel') == 'submarine' ? 'selected' : '' }}>Submarine (Bawah Air)</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status" id="status" 
                                class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500">
                                <option value="aktif" {{ old('status') == 'aktif' ? 'selected' : '' }}>Aktif (Normal)</option>
                                <option value="gangguan" {{ old('status') == 'gangguan' ? 'selected' : '' }}>Gangguan (Cut/Loss)</option>
                        ...
                        <div>
                            ...
                            <textarea name="keterangan" id="keterangan" rows="3" placeholder="Tambahkan informasi spesifikasi kabel, merk, atau rincian lokasi..."
                                class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500">{{ old('keterangan') }}</textarea>
                        </div>
                        ...
                        <div class="pt-4 flex justify-end space-x-3">
                            ...
                            <button type="submit" class="px-6 py-2.5 bg-cyan-600 hover:bg-cyan-700 text-white font-semibold rounded-lg shadow-md transition">
                                <i class="fas fa-save mr-2"></i>Simpan Backbone
                            </button>
                        </div>
                    </div>
                </form>
