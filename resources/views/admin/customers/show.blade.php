        ...
        <div class="p-6">
            <div class="mb-6">
                <div class="flex items-center space-x-4">
                    <a href="{{ route('admin.customers.index') }}" class="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">
                        <i class="fas fa-arrow-left text-xl"></i>
                    </a>
                    <div class="flex-1">
                        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ $customer->name }}</h1>
                        <p class="text-gray-600 dark:text-gray-400 mt-1">{{ $customer->username ?? 'No username' }}</p>
                    </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2 space-y-6">
                    <div class="bg-white dark:bg-slate-700 rounded-xl shadow-md p-6">
                        <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-4">
                            <i class="fas fa-user mr-2 text-cyan-600"></i>Customer Information
                        </h2>
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <p class="font-medium text-gray-900 dark:text-white">{{ $customer->username ?? '-' }}</p>
                            </div>
                            <div>
                            ...
                            <div>
                                <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Join Date</p>
                                <p class="font-medium text-gray-900 dark:text-white">{{ $customer->join_date ? $customer->join_date->format('d M Y') : '-' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white dark:bg-slate-700 rounded-xl shadow-md p-6">
                        ...
                        <div class="flex items-center justify-between p-4 bg-gradient-to-r from-cyan-50 to-blue-50 dark:from-slate-600 dark:to-slate-700 rounded-lg border border-cyan-200 dark:border-slate-600">
                            <div>
                                <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ $customer->package->name }}</h3>
                                <p class="text-sm text-gray-600 dark:text-gray-300">{{ $customer->package->speed }} Mbps</p>
                            </div>
                            <div class="text-right">
                                <p class="text-2xl font-bold text-cyan-600">Rp {{ number_format($customer->package->price, 0, ',', '.') }}</p>
                                <p class="text-sm text-gray-600 dark:text-gray-300">per month</p>
                            </div>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="bg-white dark:bg-slate-700 rounded-xl shadow-md p-6">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <a href="{{ route('admin.customers.invoices', $customer) }}" class="text-cyan-600 dark:text-cyan-400 hover:text-cyan-800 text-sm font-medium">
                                View All <i class="fas fa-arrow-right ml-1"></i>
                            </a>
                        </div>
                        <div class="space-y-3">
                            @forelse($customer->invoices()->latest()->limit(5)->get() as $invoice)
                                <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-slate-600 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-500 transition">
                                    <div>
                                        <p class="font-medium text-gray-900 dark:text-white">{{ $invoice->invoice_number }}</p>
                                        <p class="text-sm text-gray-600 dark:text-gray-300">{{ $invoice->created_at->format('d M Y') }}</p>
                                    </div>
                                    <div class="text-right">
                ...
                <div class="space-y-6">
                    <div class="bg-white dark:bg-slate-700 rounded-xl shadow-md p-6">
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center justify-between p-3 bg-blue-50 dark:bg-slate-600 rounded-lg">
                                <div class="flex items-center">
                                    <i class="fas fa-file-invoice text-blue-600 mr-3"></i>
                                    <span class="text-sm text-gray-700 dark:text-gray-200">Total Invoices</span>
                                </div>
                                <span class="font-bold text-gray-900 dark:text-white">{{ $stats['total_invoices'] }}</span>
                            </div>
                            <div class="flex items-center justify-between p-3 bg-green-50 dark:bg-slate-600 rounded-lg">
                                <div class="flex items-center">
                                    <i class="fas fa-check-circle text-green-600 mr-3"></i>
                                    <span class="text-sm text-gray-700 dark:text-gray-200">Paid</span>
                                </div>
                                <span class="font-bold text-gray-900 dark:text-white">{{ $stats['paid_invoices'] }}</span>
                            </div>
                            <div class="flex items-center justify-between p-3 bg-yellow-50 dark:bg-slate-600 rounded-lg">
                                <div class="flex items-center">
                                    <i class="fas fa-clock text-yellow-600 mr-3"></i>
                                    <span class="text-sm text-gray-700 dark:text-gray-200">Unpaid</span>
                                </div>
                                <span class="font-bold text-gray-900 dark:text-white">{{ $stats['unpaid_invoices'] }}</span>
                            </div>
                        </div>
                    </div>

                    <div class="bg-white dark:bg-slate-700 rounded-xl shadow-md p-6">
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Financial Summary</h3>
                        <div class="space-y-4">
                            <div>
                                <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Total Paid</p>
                                <p class="text-2xl font-bold text-green-600">Rp {{ number_format($stats['total_paid'], 0, ',', '.') }}</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Total Unpaid</p>
