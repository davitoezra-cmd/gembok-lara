...
@section('content')
<div class="min-h-screen bg-gray-100 dark:bg-slate-800 transition-colors duration-300" x-data="{ sidebarOpen: false }">
    ...
    <div class="lg:pl-64">
        ...
        <div class="p-6">
            ...
            <div class="max-w-4xl">
                <div class="bg-white dark:bg-slate-700 rounded-xl shadow-md p-8">
                    <div class="flex justify-between items-start mb-8 pb-6 border-b dark:border-slate-600">
                        <div>
                            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ companyName() }}</h2>
                            <p class="text-gray-600 dark:text-gray-400 mt-2">ISP Management System</p>
                        </div>
                        <div class="text-right">
                            <span class="px-4 py-2 rounded-full text-sm font-semibold {{ $invoice->status === 'paid' ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-300' }}">
                    ...
                    <div class="grid grid-cols-2 gap-8 mb-8">
                        <div>
                            <h3 class="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-3">BILL TO:</h3>
                            <p class="font-bold text-gray-900 dark:text-white">{{ $invoice->customer->name }}</p>
                            <p class="text-gray-600 dark:text-gray-400">{{ $invoice->customer->phone }}</p>
                            <p class="text-gray-600 dark:text-gray-400">{{ $invoice->customer->email }}</p>
                            <p class="text-gray-600 dark:text-gray-400 mt-2">{{ $invoice->customer->address }}</p>
                        </div>
                        <div class="text-right">
                            <div class="mb-4">
                                <p class="text-sm text-gray-600 dark:text-gray-400">Invoice Number</p>
                                <p class="font-mono font-bold text-gray-900 dark:text-white">{{ $invoice->invoice_number }}</p>
                            </div>
                            <div class="mb-4">
                    ...
                    <div class="mb-8">
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-slate-600">
                                <tr>
                                    <td class="px-4 py-4">
                                        <p class="font-medium text-gray-900 dark:text-white">{{ $invoice->package->name ?? 'Service' }}</p>
                                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ ucfirst($invoice->invoice_type) }}</p>
                                    </td>
                            ...
                            <tfoot class="bg-gray-50 dark:bg-slate-800">
                                <tr>
                                    <td class="px-4 py-4 text-right font-bold text-gray-900 dark:text-white">TOTAL</td>
                                    <td class="px-4 py-4 text-right font-bold text-2xl text-gray-900 dark:text-white">
