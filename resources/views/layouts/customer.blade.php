    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <nav class="p-4 space-y-2">
            <a href="{{ route('customer.dashboard') }}"
               class="flex items-center px-4 py-3 text-gray-300 rounded-lg hover:bg-slate-700 transition-colors {{ request()->routeIs('customer.dashboard') ? 'bg-slate-700 text-white' : '' }}">
                <i class="fas fa-home w-5 mr-3"></i><span>Dashboard</span>
            </a>
            <a href="{{ route('customer.invoices') }}"
               class="flex items-center px-4 py-3 text-gray-300 rounded-lg hover:bg-slate-700 transition-colors {{ request()->routeIs('customer.invoices*') ? 'bg-slate-700 text-white' : '' }}">
                <i class="fas fa-file-invoice w-5 mr-3"></i><span>Tagihan</span>
            </a>
            <a href="{{ route('customer.payments') }}"
               class="flex items-center px-4 py-3 text-gray-300 rounded-lg hover:bg-slate-700 transition-colors {{ request()->routeIs('customer.payments*') ? 'bg-slate-700 text-white' : '' }}">
                <i class="fas fa-credit-card w-5 mr-3"></i><span>Pembayaran</span>
            </a>
            <a href="{{ route('customer.usage') }}"
               class="flex items-center px-4 py-3 text-gray-300 rounded-lg hover:bg-slate-700 transition-colors {{ request()->routeIs('customer.usage*') ? 'bg-slate-700 text-white' : '' }}">
                <i class="fas fa-chart-line w-5 mr-3"></i><span>Pemakaian</span>
            </a>
            <a href="{{ route('customer.tickets') }}"
               class="flex items-center px-4 py-3 text-gray-300 rounded-lg hover:bg-slate-700 transition-colors {{ request()->routeIs('customer.tickets*') ? 'bg-slate-700 text-white' : '' }}">
                <i class="fas fa-ticket-alt w-5 mr-3"></i><span>Tiket Saya</span>
            </a>
            <a href="{{ route('customer.profile') }}"
               class="flex items-center px-4 py-3 text-gray-300 rounded-lg hover:bg-slate-700 transition-colors {{ request()->routeIs('customer.profile*') ? 'bg-slate-700 text-white' : '' }}">
                <i class="fas fa-user-cog w-5 mr-3"></i><span>Profil</span>
            </a>
            <a href="{{ route('customer.support') }}"
               class="flex items-center px-4 py-3 text-gray-300 rounded-lg hover:bg-slate-700 transition-colors {{ request()->routeIs('customer.support*') ? 'bg-slate-700 text-white' : '' }}">
                <i class="fas fa-headset w-5 mr-3"></i><span>Bantuan</span>
            </a>
        </nav>
        ...
        <header class="bg-white dark:bg-slate-900 shadow-sm border-b border-gray-200 dark:border-slate-700 sticky top-0 z-30 transition-colors duration-300">
            <div class="flex items-center justify-between px-4 py-3">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <span class="text-gray-600 dark:text-gray-300">{{ $customer->name ?? 'Customer' }}</span>
                    <div class="w-10 h-10 bg-cyan-100 dark:bg-cyan-900 rounded-full flex items-center justify-center">
