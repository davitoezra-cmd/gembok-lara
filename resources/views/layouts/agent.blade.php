    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <nav class="p-4 space-y-2">
            <a href="{{ route('agent.dashboard') }}"
               class="flex items-center px-4 py-3 text-gray-300 rounded-lg hover:bg-slate-700 transition-colors {{ request()->routeIs('agent.dashboard') ? 'bg-slate-700 text-white' : '' }}">
                <i class="fas fa-home w-5 mr-3"></i><span>Dashboard</span>
            </a>
            <a href="{{ route('agent.vouchers.sell') }}"
               class="flex items-center px-4 py-3 text-gray-300 rounded-lg hover:bg-slate-700 transition-colors {{ request()->routeIs('agent.vouchers*') ? 'bg-slate-700 text-white' : '' }}">
                <i class="fas fa-ticket w-5 mr-3"></i><span>Jual Voucher</span>
            </a>
            <a href="{{ route('agent.topup') }}"
               class="flex items-center px-4 py-3 text-gray-300 rounded-lg hover:bg-slate-700 transition-colors {{ request()->routeIs('agent.topup*') ? 'bg-slate-700 text-white' : '' }}">
                <i class="fas fa-plus-circle w-5 mr-3"></i><span>Top Up Saldo</span>
            </a>
            <a href="{{ route('agent.transactions') }}"
               class="flex items-center px-4 py-3 text-gray-300 rounded-lg hover:bg-slate-700 transition-colors {{ request()->routeIs('agent.transactions*') ? 'bg-slate-700 text-white' : '' }}">
                <i class="fas fa-history w-5 mr-3"></i><span>Riwayat Transaksi</span>
            </a>
            <a href="{{ route('agent.profile') }}"
               class="flex items-center px-4 py-3 text-gray-300 rounded-lg hover:bg-slate-700 transition-colors {{ request()->routeIs('agent.profile*') ? 'bg-slate-700 text-white' : '' }}">
        ...
        <header class="bg-white dark:bg-slate-900 shadow-sm border-b border-gray-200 dark:border-slate-700 sticky top-0 z-30 transition-colors duration-300">
            <div class="flex items-center justify-between px-4 py-3">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <span class="text-gray-600 dark:text-gray-300">{{ Auth::user()->name ?? 'Agent' }}</span>
